 ...
 * 
 * @note L'idée consiste ici à incrémenter le compteur à chaque fois
 *       que la broche de lecture du bouton détecte un changement d'état.
 *       
 *       Autrement dit, à chaque fois que vous enfoncez ou relâchez le
 *       bouton, le compteur est incrémenté. Mais il l'est également
 *       lorsqu'un effet rebond survient !
 *       
 *       Donc, surveillez bien l'affichage du compteur lorsque vous agissez
 *       sur le bouton poussoir. Vous devriez normalement observer des "sauts"
 *       intempestifs.
 */
void loop() {
    ...
    // c'est qu'il y'a eu un changement d'état...
    if (btn_state != last_btn_state) {

        // Alors on incrémente le compteur et on affiche
        // sa valeur sur la rampe de LEDs.
        ledWrite(++counter);
        // Par conséquent, si vous voyez qu'après avoir enfoncé
        // ou relâché le bouton le compteur affiché a varié de plus
